    ...
    def __import_type_support__(cls):
        try:
            ...
            logger = logging.getLogger(
                'control_interfaces.msg.VelocityCommand')
            logger.debug(
...
class VelocityCommand(metaclass=Metaclass_VelocityCommand):
    ...
    def __init__(self, **kwargs):
        ...
        self.command = array.array('d', kwargs.get('command', []))
    ...
    def command(self, value):
        if isinstance(value, array.array):
            assert value.typecode == 'd', \
                "The 'command' array.array() must have the type code of 'd'"
            self._command = value
            return
        if __debug__:
            ...
            assert \
                ...
                "The 'command' field must be a set or sequence and each value of type 'float' and each double in [-.000000, .000000]"
        self._command = array.array('d', value)
